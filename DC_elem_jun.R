...
stargazer(coeftest(reg1_elem),coeftest(reg2_elem),coeftest(reg3_elem),coeftest(reg4_elem),coeftest(reg5_elem),coeftest(reg1_jun),coeftest(reg2_jun),coeftest(reg3_jun),coeftest(reg4_jun),coeftest(reg5_jun),
          ...
          digits=2,
          column.labels = c("DC","DC","DC","DC","DC","DC","DC","DC","DC","DC"),
          ...
          title="Survey Double Hurdle Regression of Willingness to Accept for a School Merger for Elementary and Junior High Schools Separately Using Dichotomous Choice Answers",
          
          keep.stat="n",
          add.lines = list(c("Observations",rep(n_elem,5),rep(n_jun,5))),
          type="latex",out=paste0(path_results,"dc_elem_jun.tex"))
